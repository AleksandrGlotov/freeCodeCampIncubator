...
Window.prototype.tabClose = function (index) {
  ...
  console.log("1", tabsBeforeIndex);
  const tabsAfterIndex = this.tabs.slice(index + 1); // Get the tabs after the tab
  console.log("2", tabsAfterIndex);
...
const filteredList = watchList1
  .filter((item) => Number(item["imdbRating"]) > 8)
...
function nonMutatingConcat(original, attach) {
  ...
  // Only change codear above this line
}
...
function getRating(watchList) {
  // Only change code below this line
  let averageRating;
  let sumRating = watchList
    .filter((i) => i["Director"] == "Christopher Nolan")
    .reduce((sum, film) => sum + Number(film["imdbRating"]), 0);
  averageRating =
    sumRating /
